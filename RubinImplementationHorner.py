...
def hornerBerechnung(x: float, n: int) -> float:
    a0 = 1/math.factorial(n)
    for k in range(n-1,-1,-1):
        a0 = a0 * x + (1/math.factorial(k))
    return a0
...
for i in range(0,10):
    index.append((i+1)*3)
    methode1[i] = hornerBerechnung(-5.5, index[i])
    methode2[i] = 1/hornerBerechnung(5.5, index[i])
    methode3[i] = hornerBerechnung(-0.5, index[i])**11
